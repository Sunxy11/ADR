...
def _load_samples(source_pth, target_pth):
    ...
    with open(source_pth, 'r') as fp:
        ...
        for row in rows:
            ...
            imagea_list.append(row[:index+9])
    with open(target_pth, 'r') as fp:
        rows = fp.readlines()
        for row in rows:
            index = row.find('tfrecords')
            imageb_list.append(row[:index+9])
...
def load_data(source_pth, target_pth, do_shuffle=True):
    ...
    if do_shuffle is True:
        img_i, img_j, gt_i, gt_j = tf.train.shuffle_batch([image_i, image_j, gt_i, gt_j], BATCH_SIZE, capacity=500, min_after_dequeue=100)
        
    else:
        img_i, img_j, gt_i, gt_j = tf.train.batch([image_i, image_j, gt_i, gt_j], batch_size=BATCH_SIZE, num_threads=1, capacity=500)
    return img_i, img_j, gt_i, gt_j
